...
freq_counter = []
for minute in range(0,151):
    if (minute in combined) == True:
        freq_counter.append(combined.count(minute))
    else:
        freq_counter.append(0)
...
republican = mpatches.Patch(color='green', label='Other')
democrat = mpatches.Patch(color='blue', label='pro-Obama and pro-McCain')



# y = np.row_stack((proobama,promccain))
y = np.row_stack((RepDem,other))
x = range(0,151)
...
ax.set_title('Opinions in tweets over time', y=1.06)
...
# plt.show()
savefig('other&politicalStackPlot.pdf')